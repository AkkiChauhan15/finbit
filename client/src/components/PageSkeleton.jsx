  ...
  return (
    <div className="animate-pulse space-y-8" role="status" aria-label={label}>
      ...
      <div className="space-y-3">
        ...
        <div className="h-9 w-3/4 max-w-sm rounded bg-slate-800" />
        <div className="h-4 w-full max-w-xl rounded bg-slate-900" />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        {Array.from({ length: sections }, (_, index) => (
          <div key={index} className="h-72 rounded-2xl border border-slate-800 bg-slate-900" />
        ))}
      </div>
    </div>
  );
